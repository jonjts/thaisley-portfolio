...
import { Container, CardContainer, CardColumn, CardRow } from './styles';
...
const Content: React.FC = () => {
  ...
  const Interfaces = () => (
    <CardRow>
      {interfaces.map((item, key) => (
        <CardColumn xs={12} sm={12} md={12} lg={4} key={key}>
          <Card
            imageSorce={item.image}
            categoria={t(item.categoria)}
            title={item.title}
            onClick={() => {
              navigate(`/cases?key=${item.id}`);
            }}
          />
        </CardColumn>
      ))}
  ...
  return (
    <Container id="jobs">
      ...
      <ContactButtonContainer style={{ marginBottom: 113 }} />
    </Container>
  );
};
